...
**CRITICAL**: ≥75% of P0/P1 stakeholder requirements MUST have associated test cases PASSED.
...
|----------------|----------|-------|------------|--------|----------|
| STR-XXX-NNN | P0/P1/P2 | Requirement title | test_file.cpp::test_name | ✅/❌/⚠️ | % |

**Status Legend**:
- ✅ **PASS**: All test cases passing
- ❌ **FAIL**: One or more test cases failing
- ⚠️ **PARTIAL**: Some tests passing, some failing
- 📋 **NO TESTS**: No test cases linked to requirement

### Coverage Calculation

```
Requirement Coverage = (Passing tests for requirement) / (Total tests for requirement) × 100%
```
...
|-----------|-----------|--------|----------|
| `tests/test_bmca_tdd.cpp` | bmca_tdd_tests::dataset_comparison_priority1 | ✅ | [timestamp] |
| `tests/test_bmca_tdd.cpp` | bmca_tdd_tests::dataset_comparison_clockClass | ✅ | [timestamp] |
| `tests/test_bmca_tdd.cpp` | bmca_tdd_tests::dataset_comparison_clockAccuracy | ✅ | [timestamp] |
| `tests/test_bmca_tdd.cpp` | bmca_tdd_tests::dataset_comparison_variance | ✅ | [timestamp] |
| `tests/test_bmca_tdd.cpp` | bmca_tdd_tests::dataset_comparison_priority2 | ✅ | [timestamp] |
| `tests/test_bmca_tdd.cpp` | bmca_tdd_tests::dataset_comparison_clockIdentity | ✅ | [timestamp] |
| `tests/test_offset_calculation.cpp` | offset_calc::basic_calculation | ✅ | [timestamp] |
| `tests/test_offset_calculation.cpp` | offset_calc::asymmetric_delay | ✅ | [timestamp] |
| `tests/test_configuration_setters.cpp` | config::set_announce_interval | ✅ | [timestamp] |
| `tests/test_configuration_setters.cpp` | config::set_sync_interval | ✅ | [timestamp] |
| ... | ... | ... | ... |
...
| Test File | Test Case | Status | Last Run |
|-----------|-----------|--------|----------|
| `tests/test_msg_announce_validation.cpp` | msg_announce::field_order | ✅ | [timestamp] |
| `tests/test_msg_announce_validation.cpp` | msg_announce::network_byte_order | ✅ | [timestamp] |
| `tests/test_msg_sync_validation.cpp` | msg_sync::header_format | ✅ | [timestamp] |
| ... | ... | ... | ... |
...
| Test File | Test Case | Status | Last Run |
|-----------|-----------|--------|----------|
| `tests/test_bmca_tdd.cpp` | bmca_tdd_selection::master_selection_by_priority | ✅ | [timestamp] |
| `tests/test_bmca_tdd.cpp` | bmca_tdd_selection::state_stability | ✅ | [timestamp] |
| ... | ... | ... | ... |
...
| Test File | Test Case | Status | Last Run |
|-----------|-----------|--------|----------|
| *(none)* | *(manual testing required)* | 📋 | N/A |

**Coverage**: 0% (requires physical hardware)  
**Status**: 📋 NO TESTS (manual test procedure documented)  
...
| Test File | Test Case | Status | Last Run |
|-----------|-----------|--------|----------|
| `tests/test_offset_calculation.cpp` | offset_calc::accuracy_validation | ✅ | [timestamp] |
| `tests/test_offset_calculation.cpp` | offset_calc::nanosecond_precision | ✅ | [timestamp] |
| ... | ... | ... | ... |

**Coverage**: 75% (calculation correctness validated, full accuracy requires hardware)  
**Status**: ⚠️ PARTIAL (simulation tests pass, hardware validation pending)
...
| Test File | Test Case | Status | Last Run |
|-----------|-----------|--------|----------|
| *(CI symbol analysis)* | no_dynamic_allocation_check | ✅ | [timestamp] |
...
**Acceptance Criteria**:
- Offset SHALL converge to <1µs within 60 seconds
...
| Test File | Test Case | Status | Last Run |
|-----------|-----------|--------|----------|
| *(none)* | *(simulation test needed)* | 📋 | N/A |
...
**Acceptance Criteria**:
- Mean delay within ±10% of cable propagation delay
...
| Test File | Test Case | Status | Last Run |
|-----------|-----------|--------|----------|
| `tests/test_master_delay_response.cpp` | delay::path_delay_calculation | ✅ | [timestamp] |
| `tests/test_master_delay_response.cpp` | delay::delay_response_handling | ✅ | [timestamp] |
| ... | ... | ... | ... |
...
| Test File | Test Case | Status | Last Run |
|-----------|-----------|--------|----------|
| *(CI header analysis)* | no_platform_headers_check | ✅ | [timestamp] |
...
| Test File | Test Case | Status | Last Run |
|-----------|-----------|--------|----------|
| `tests/test_msg_announce_validation.cpp` | validation::invalid_packet_length | ✅ | [timestamp] |
| `tests/test_msg_announce_validation.cpp` | validation::corrupted_fields | ✅ | [timestamp] |
| ... | ... | ... | ... |

**Coverage**: 80% (validation tests pass, full fuzzing not yet in CI)  
**Status**: ⚠️ PARTIAL (validation tests pass, AFL++ fuzzing pending)
...
| Test File | Test Case | Status | Last Run |
|-----------|-----------|--------|----------|
| *(CI example build)* | ieee1588_2019_basic_example::compile | ✅ | [timestamp] |
| *(CI example build)* | ieee1588_2019_realtime_example::compile | ✅ | [timestamp] |
| *(CI example run)* | ieee1588_2019_basic_example::execute | ✅ | [timestamp] |
...
| Test File | Test Case | Status | Last Run |
|-----------|-----------|--------|----------|
| *(All unit tests)* | *(entire test suite)* | ✅ | [timestamp] |
...
**Stakeholder Requirement Coverage**: **83.3%** ✅ (exceeds 75% threshold)
...
### Requirements → REQ-* Design Requirements (Maintained)

**NOTE**: STR-* → REQ-* links are preserved in requirement specifications.
...
- id: STR-STD-001
  linkedRequirements:
    - REQ-FUNC-001  # Message handling
    - REQ-FUNC-002  # BMCA implementation
    - REQ-FUNC-005  # Offset calculation
```
...
STR-STD-001 (Stakeholder)
  ...
  │    │    ├─> DES-CLK-001 (Detailed Design)
  │    │    │    └─> src/clocks.cpp::process_sync_message() (Implementation)
  │    │    │         └─> tests/test_offset_calculation.cpp (Tests)
  │    └─> REQ-PERF-001 (Performance Requirement)
  └─> Test: test_offset_calculation.cpp::offset_calc::basic_calculation ✅
```
...
**Exit Codes**:
- `0`: Coverage ≥75%, all validations pass
